...
use App\Filament\Resources\PengajuanResource\RelationManagers\ItemsRelationManager;
use App\Filament\Resources\PengajuanResource\RelationManagers\LogsRelationManager;
                ...
                Forms\Components\Section::make('Informasi Pengajuan')
                    ...
                    ->schema([
                        Forms\Components\Placeholder::make('nomor')
                            ->label('Nomor Dokumen')
                            ->content(fn(?Pengajuan $record) => $record?->nomor ?? '(otomatis saat simpan)')
                            ->columnSpan(4),

                        Forms\Components\Placeholder::make('status')
                            ->label('Status')
                            ->content(fn(?Pengajuan $record) => $record?->status ? ucfirst($record->status) : 'Draft')
                            ->columnSpan(3),

                        Forms\Components\Placeholder::make('creator')
                            ->label('Dibuat Oleh')
                            ->content(fn(?Pengajuan $record) => optional($record?->creator)->name ?? '-')
                            ->columnSpan(5),

                        Forms\Components\Placeholder::make('total_pokok')
                            ->label('Total Pokok')
                            ->content(fn(?Pengajuan $record) => static::formatRp($record?->total_pokok ?? 0))
                            ->columnSpan(4),

                        Forms\Components\Placeholder::make('total_admin')
                            ->label('Total Admin')
                            ->content(fn(?Pengajuan $record) => static::formatRp($record?->total_admin ?? 0))
                            ->columnSpan(4),

                        Forms\Components\Placeholder::make('grand_total')
                            ->label('Grand Total')
                            ->content(fn(?Pengajuan $record) => static::formatRp($record?->grand_total ?? 0))
                            ->columnSpan(4),

                        Forms\Components\Placeholder::make('timestamps')
                            ->label('Timestamps')
                            ->content(function (?Pengajuan $record) {
                                if (! $record) return '-';
                                $parts = [];
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->columns([
                ...
                Tables\Columns\TextColumn::make('total_pokok')
                    ->label('Total Pokok')
